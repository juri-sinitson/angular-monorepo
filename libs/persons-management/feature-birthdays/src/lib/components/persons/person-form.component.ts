        ...
        <p-messages severity="info">
          <ng-template pTemplate>
            <div data-testid="info-29-february">
              <b>NOTE!</b>
              <p>This date exists in a leap year only.</p>
            </div>
          </ng-template>
        </p-messages>
        } @if(!changesMade()) {
        ...
        <p-messages severity="error">
          <ng-template pTemplate>
            <div data-testid="entity-already-exists">
              ...
              <p>
                It looks like you already have the person with the same name,
                surname and birth date in the database.
              </p>
            </div>
          </ng-template>
        </p-messages>
          ...
          <ng-template pTemplate>
            <div data-testid="date-does-not-exist">
              ...
              <p>
                ...
                of day, month and year. E.g. 31th of April does not exist and
                29th of February exists in a leap year only.
              </p>
            </div>
          </ng-template>
        </p-messages>
...
export class PersonFormComponent extends AbstractEntityFormComponent<EntityInterface> {
  ...
  is29FebruaryWithoutYear(): boolean {
    const day: number | null = this.getControlValueByPath(
      'birthDay',
      this.entityForm
    );
    const month: number | null = this.getControlValueByPath(
      'birthMonth',
      this.entityForm
    );
    const year: number | null = this.getControlValueByPath(
      'birthYear',
      this.entityForm
    );
  ...
  protected override getExistingEntity(): EntityInterface | undefined {
    return (this.allData() as EntityInterface[]).find(
      (person: EntityInterface) => {
        ...
        return (
          person.name === this.getControlValueByPath('name', this.entityForm) &&
          areSurnamesEqual &&
          person.birthDay ===
            this.getControlValueByPath('birthDay', this.entityForm) &&
          person.birthMonth ===
            this.getControlValueByPath('birthMonth', this.entityForm) &&
          areBirthYearsEqual
        );
      }
    );
  ...
  protected override initForm(dataArg: EntityInterface | null): void {
    ...
    this.entityForm.addControl(
      'id',
      new FormControl<string | number>('0', { nonNullable: true })
    );
    this.entityForm.addControl(
      'name',
  ...
  dateExists(): boolean | null {
    const birthDay: number | null = this.getControlValueByPath(
      'birthDay',
      this.entityForm
    );
    const birthMonth: number | null = this.getControlValueByPath(
      'birthMonth',
      this.entityForm
    );
    const birthYear: number | null = this.getControlValueByPath(
      'birthYear',
      this.entityForm
    );
