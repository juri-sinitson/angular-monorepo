...
To avoid versioning of tokes, passwords and other sensitive data.
...
1. Create the copy of the dist-file without dist (in this example copy `nx.json.dist`  to 
`nx.json`).
2. Replace the placeholder mentioned above with your sensitive value.
3. The file without dist shouldn't be versioned! Ensure once more it's the case!
If not, contact your repo admin. 


## The list
1. `nx.json.dist` - this file contains the token for the nx workspace in the cloud

# Generated docs: AngularMonorepo
